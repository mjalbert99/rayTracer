...
void Scene::init(void) {
    ...
    model["table piece"]->material = material["ceramic"];

    model["table piece2"] = new Model;
    model["table piece2"]->geometry = geometry["cube"];
    model["table piece2"]->material = material["turquoise"];
    ...
    model["concrete"] = new Model;
    model["concrete"]->geometry = geometry["cube"];
    model["concrete"]->material = material["silver"];
    ...
    node["floor"] = new Node;
    node["concrete"] = new Node;


    node["table"]->childnodes.push_back(node["table top"]);
    node["table"]->childtransforms.push_back(translate(vec3(-1.0f, 1.3f, 0.0f)) * rotate(120.0f * float(M_PI) / 180.0f, vec3(0.0f, 1.0f, 0.0f)));
    node["table"]->childnodes.push_back(node["table leg"]);
    node["table"]->childtransforms.push_back(translate(vec3(-0.8f, 0.4f, 2.4f)) * rotate(120.0f * float(M_PI) / 180.0f, vec3(0.0f, 1.0f, 0.0f)));
    //node["table"]->childnodes.push_back(node["teapot2"]);
    //node["table"]->childtransforms.push_back(translate(vec3(0.2f, 0.1f, 1.6f)) * rotate(200.0f * float(M_PI) / 180.0f, vec3(0.0f, 1.0f, 0.0f)));


    node["floor"]->childnodes.push_back(node["concrete"]);
    node["floor"]->childtransforms.push_back(translate(vec3(0.0f, 0.1f, 0.0f)));

    node["floor"]->models.push_back(model["concrete"]);
    node["floor"]->modeltransforms.push_back(translate(vec3(0.0f, -0.0f, 0.0f)) * scale(vec3(10.0f, .1f, 10.0f)));

    node["table top"]->models.push_back(model["table piece"]);
    node["table top"]->modeltransforms.push_back(translate(vec3(0.0f, -0.1f, 0.0f)) * scale(vec3(2.0f, 2.0f, 2.0f)));

    node["table leg"]->models.push_back(model["table piece2"]);
    node["table leg"]->modeltransforms.push_back(translate(vec3(0.2f, 0.1f, 1.6f)) * scale(vec3(1.0f, 1.0f, 1.0f)));
    ...
    camera->eye_default = vec3(0.0f, 2.0f, 10.0f);
